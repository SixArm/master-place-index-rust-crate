...
COPY --from=builder /app/target/release/master_place_index /app/master_place_index-server
...
# Run the application
CMD ["/app/master_place_index-server"]
```
...
services:
  ...
  # Master Place Index application
  mpi-server:
    build:
      context: .
  ...
  test-runner:
    ...
    depends_on:
      postgres-test:
        condition: service_healthy
    environment:
      DATABASE_URL: postgresql://test_user:test_password@postgres-test:5432/master_place_index_test
...
```bash
# Database
DATABASE_URL=postgresql://master_place_index_user:mpi_password@localhost:5432/master_place_index
...
DATABASE_URL=postgresql://master_place_index_prod:STRONG_PASSWORD@db-host:5432/master_place_index_production?sslmode=require
...
RUST_LOG=info,master_place_index=info
RUST_BACKTRACE=0
```
  ...
  test:
    ...
    steps:
      ...
      - name: Push to Registry
        ...
        run: |
          docker tag mpi-server:${{ github.sha }} registry.io/master_place_index-server:latest
          docker push registry.io/master_place_index-server:latest
```
...
OTLP_SERVICE_NAME=master-place-index
```
