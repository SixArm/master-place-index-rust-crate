...
```bash
DATABASE_URL=postgresql://test_user:test_password@localhost:5432/master_place_index_test
...
```bash
kubectl rollout undo deployment/master_place_index-server
```
...
```yaml
kubectl set image deployment/master_place_index-server mpi-server=${{ env.REGISTRY }}/${{ env.IMAGE_NAME }}:${{ github.ref_name }}
```
...
# Pull image
docker pull ghcr.io/username/master-place-index-rust-crate:latest
```
...
# Kubernetes
kubectl rollout undo deployment/master_place_index-server
kubectl rollout undo deployment/master_place_index-server --to-revision=3
...
Phase 13 completes the Master Place Index project with a production-ready CI/CD pipeline. The system now has automated testing, code quality checks, security scanning, and deployment automation, making it ready for production use in geographic data management environments.
